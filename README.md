...
## 협업 시 충돌(Conflict) 대응 절차

협업 중 브랜치 충돌이 발생하면 아래 순서로 업데이트 후 작업을 재개합니다.

1. 원격 최신 이력 가져오기
   ```bash
   git fetch origin
   ```
2. 작업 브랜치에서 최신 기준 브랜치 반영
   ```bash
   git rebase origin/main
   ```
   - 또는 팀 규칙이 merge 기반이면 `git merge origin/main` 사용
3. 충돌 파일 수정 후 상태 확인
   ```bash
   git status
   ```
4. 충돌 해결 파일 스테이징
   ...
   git add <해결한_파일>
   ```
5. 리베이스 재개(또는 머지 커밋 완료)
   ```bash
   git rebase --continue
   ```
6. 실험/테스트 재실행 후 커밋/푸시

참고: 코드나 문서에 `<<<<<<<`, `=======`, `>>>>>>>` 표식이 남아 있으면 충돌이 완전히 해결되지 않은 상태이므로 반드시 제거해야 합니다.
